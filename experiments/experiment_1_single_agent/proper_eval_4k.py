...
def get_erl_names_agents():
    begin = './pickles/experiment1/erl/proper_v2/'
    ...
    for i in range(len(files)):
        ...
        if 'gen_0' in filename: continue
        eps = filename.split("_")[-1].split(".p")[0]
        names.append(f"ERL Steps {eps} ({filename.split('/')[-1]})")
        pickles.append(filename)
    agents = []
    with open(f"{begin}05-07-2021_13-32-20_ERL_whole_pop_gen_0_step_0.p", 'rb') as f:
        agents.append(pickle.load(f)['pop'].population[0])
    print(names)
    print(pickles);
    print("\n")
    ...
    for i in range(len(agents)):
        agents[i].learn = False
    names = names #[32:]
    agents = agents #[32:]
    print("ERL", len(names), len(agents))
    print(names)
    # exit()
    return names, agents
...
def eval_single(names, agents):
    ...
    for i, (name, agent) in enumerate(zip(names, agents)):
        ...
        answer = eval_on_special_levels(agent, 5, 4000)
...
def singles(val):
    ...
    if val == 'nonl':
        ...
        for N, D in zip(all_names, all_data):
            ...
            little_dict = eval_single(names, agents)
            ANS[N] = little_dict
        os.makedirs(f'./pickles/experiment1/eval/{n}', exist_ok=True)
        print(ANS)
        with open(f"./pickles/experiment1/eval/{n}/10k_eval_1_rando12_hardcode_{get_date()}.p", 'wb+') as f:
            pickle.dump(ANS, f)
        return ANS
    if val == 'step0':
    ...
    if val == 'erl':
        name = "ERL"
        print("DOING ERL")
        ans = eval_single(*get_erl_names_agents())
        ANS = {}
        ANS[name] = ans
        print(ans)
        os.makedirs(f"./pickles/experiment1/eval/{n}", exist_ok=True)
        with open(f"./pickles/experiment1/eval/{n}/{prefix}_{name}_{get_date()}_gen32_to_54.p", 'wb+') as f:
            pickle.dump(ANS, f)
def main():
    print(sys.argv[-1])
    singles(sys.argv[-1])
